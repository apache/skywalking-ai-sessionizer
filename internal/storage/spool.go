...
// SpoolDir is where OTLP metrics requests wait under a storage root, each
// derived from one landed file. Each is one file, write-once like every
// landed file, and asz push sends them in order and records which were sent.
const SpoolDir = "_metrics"

// Spool is the metrics spool of one storage root.
type Spool struct{ dir string }

// NewSpool opens the spool of a root; nothing is created until PutNamed.
