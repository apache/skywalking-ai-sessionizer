...
// Every name the model can emit has an entry, even where a body has no word
// for it, so a reader asking for native words is never shown a gap.
func TestGlossaryCoversTheWholeVocabulary(t *testing.T) {
	g := claudecodeprovider.Glossary()
	for _, name := range model.Vocabulary() {
		if _, ok := g.Lookup(name); !ok {
			t.Errorf("no glossary entry for %q", name)
		}
	}
}
