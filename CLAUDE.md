...
Apache-2.0. A subproject of Apache SkyWalking: `github.com/apache/skywalking-ai-sessionizer`.
...
| `docs/en/concepts-and-designs/glossary.md` | Every word with a fixed meaning, and what each is **not**. |
| `docs/en/formats/` | The three shapes: Session Data (`.sd`), Session Flow (`.sf`), and `asz.view` (one conversation as one document, never a file). asz defines and owns all three; a server that mirrors them, such as the OAP, follows these pages. |
| `docs/en/adapters/claude-code-glossary.md` | What Claude Code calls each of them. Generated by `asz glossary`. |
| `docs/en/adapters/claude-code.md` | How Claude Code maps onto the model, and what it cannot supply. |

There is no design-notes folder. A feature is documented in `docs/en` when it lands. The plans and
measurements that led to it stay in the git history, never as working notes in the tree.
...
immutable rounds — see the assembly doc.
...
- **Both vocabularies survive.** The model's words are the project's, but a reader can ask for the
  runtime's with `-terms native`. The adapter declares a glossary and a test fails if a name the
  model can emit has no entry - including entries that say the runtime has no word for it.
