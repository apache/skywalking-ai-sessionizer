...
| `docs/en/formats/` | The three shapes: Session Data (`.sd`), Session Flow (`.sf`), Conversation View (`cv/1`, never a file). asz defines and owns all three; a server that mirrors them, such as the OAP, follows these pages. |
...
pkg/sessionview/                Conversation View: one conversation as one document, `cv/1`; never a file
