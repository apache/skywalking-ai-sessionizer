...
# Usage: tools/ci-binaries.sh VERSION COMMIT RUN_ID OUT_DIR "OS/ARCH ..." ["DEB_PACKAGES"]
...
set -euo pipefail
[ "$#" -eq 5 ] || [ "$#" -eq 6 ] || { echo 'usage: tools/ci-binaries.sh VERSION COMMIT RUN_ID OUT_DIR "OS/ARCH ..." ["DEB_PACKAGES"]' >&2; exit 2; }
[ "$#" -eq 6 ] || set -- "$@" ""
