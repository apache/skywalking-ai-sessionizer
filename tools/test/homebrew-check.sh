...
#
#   tools/test/homebrew-check.sh VERSION PKG_DIR
#
# PKG_DIR holds the six binary packages of VERSION with their .sha512, as
# tools/release/install-manifests.sh needs them. The script writes the formulae with
...
[ "$#" -eq 2 ] || { echo "usage: tools/test/homebrew-check.sh VERSION PKG_DIR" >&2; exit 2; }
version=$1
...
bash "$tree/tools/release/install-manifests.sh" "$version" "$pkg_dir" "$work/manifests" >/dev/null
