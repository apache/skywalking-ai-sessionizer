...
#
#   tools/release/apt-repository.sh [--from DIR] [--check] APT_DIR VERSION...
#
...
# by a key in KEYS. With --check, tools/test/apt-check.sh installs them with apt
# before anything is written, which needs docker.
#
# tools/release/apt-index then adds them to the index in APT_DIR, and writes the
...
[ "$#" -ge 2 ] || { echo "usage: tools/release/apt-repository.sh [--from DIR] [--check] APT_DIR VERSION..." >&2; exit 2; }
...
if [ "$check" = true ]; then
  ...
  bash "$tree/tools/test/apt-check.sh" "$work/packages" "$@" || fail "tools/test/apt-check.sh failed, so $apt was not changed"
fi
