...
# this repository is.
#
#   tools/release/homebrew-formula.sh [--from DIR] [--check] VERSION...
#
...
# tools/release/install-manifests.sh holds each package to its .sha512 and writes the
# formulae.
#
...
#
# With --check, each version's formulae go through tools/test/homebrew-check.sh,
...
[ "$#" -ge 1 ] || { echo "usage: tools/release/homebrew-formula.sh [--from DIR] [--check] VERSION..." >&2; exit 2; }
for v in "$@"; do
...
for v in "$@"; do
  ...
  bash "$tree/tools/release/install-manifests.sh" "$v" "$dir" "$work/manifests-$v" >/dev/null
  if [ "$check" = true ]; then
    say "checking the formulae of $v with Homebrew"
    bash "$tree/tools/test/homebrew-check.sh" "$v" "$dir" || fail "the formulae of $v did not pass tools/test/homebrew-check.sh, so none of $v was written"
  fi
  mkdir -p "$formula"
